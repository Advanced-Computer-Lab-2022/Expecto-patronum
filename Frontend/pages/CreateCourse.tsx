        ...
        <div className="col">
          ...
          <div className="p-2">
            <textarea
              ref={textareaRef}
              required
              onChange={(e) => setCharacterLeft(250 - e.target.value.length)}
              onFocus={moveLabel}
              onBlur={(e) => returnToInitial(e)}
              maxLength={250}
              className="create-course-input h-32 py-2 focus:outline-none resize-none w-full transition-bg duration-1000 text-white bg-transparent rounded-lg pl-2 shadow-lg border-2 border-navlink-bg"
            ></textarea>
            <label
              ref={labelRef}
              className="create-course-input-label relative h-4 whitespace-nowrap block w-fit pointer-events-none rounded-lg bottom-32 text-gray-400 left-3 bg-navbar px-1 duration-1000"
            >
              Summary *
            </label>
            <label className="relative text-sm text-gray-400 bottom-5 mr-3 float-right">
              {characterLeft} character{characterLeft === 1 ? "" : "s"} left
            </label>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto w-700 rounded-b-2xl bg-navbar">
        ...
        <div className="text-center flex justify-center ">
          ...
          <button
            ...
            type="button"
            onClick={goToNext}
            className="inline-flex h-10 mb-4 items-center py-2 px-4 ml-3 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-800 hover:bg-blue-800 dark:bg-blue-600 dark:hover:bg-blue-700"
          >
            Next
            <svg
              aria-hidden="true"
