...
const Input = React.forwardRef((props: Props, ref) => {
    ...
    function returnToInitial(e: React.FocusEvent<HTMLTextAreaElement, Element> | React.FocusEvent<HTMLInputElement, Element>) {
        if(e.target.value === "") {
            ...
            labelRef.current.style.color = "rgba(0, 0, 0, 0.5)";
  ...
  if(props.type === "textarea") {
    ...
    return (
        <div ref={ref as any}  className={props.inputDivStyle + (props.required ? ' create-course-input': "") + ' p-2'}> 
            <label ref={labelRef} className={props.labelStyle + (props.required ? ' create-course-input-label ': "") + ' relative h-4 whitespace-nowrap block w-fit pointer-events-none rounded-lg top-7 left-3 bg-white text-dark-semi-transparent px-1'}>{props.placeholder}</label>
            ...
            <label className='relative text-xs text-gray-400 top-1 mr-2 float-right'>{characterLeft} character{characterLeft === 1 ? "": "s"} left</label>
        </div>
    )
            ...
            {props.enum?.map((item) => {
                return (
                    <div key={item} className={props.inputDivStyle + ' col-3 text-center px-0 mx-0 py-2'}>
                        <input onChange={props.onChange} type="radio" name={props.placeholder + "-button"} className={props.style + ' checked:border-8 checked:border-white checked:bg-navlink-bg transition-all duration-300 h-8 w-8 bg-transparent shadow-[0px_0px_5px_#0088A8] rounded-[50%] border-navlink-bg border-[0.2vw] appearance-none'}></input>
                        <br />
    ...
    return (
        <div ref={ref as any} className={props.inputDivStyle + ' flex ml-4 items-center mt-3'}>
            <label className={props.labelStyle + ' pointer-events-none text-xs h-4 whitespace-nowrap text-dark-semi-transparent px-1'}>{props.placeholder}</label>
    ...
    return (
        <div ref={ref as any} className={props.inputDivStyle + ' w-auto p-2 min-w-[8.9rem]'}>
            <label ref={labelRef} className={(props.required ? 'create-course-input-label ': "") + props.labelStyle + ' relative whitespace-nowrap block w-fit pointer-events-none rounded-lg top-7 h-4 left-2 bg-white text-dark-semi-transparent px-1'}>{props.placeholder}</label>
