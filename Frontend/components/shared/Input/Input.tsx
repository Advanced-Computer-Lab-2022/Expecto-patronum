  ...
  if(props.type === "textarea") {
    ...
    return (
        <div ref={ref as any}  className={props.inputDivStyle + (props.required ? ' create-course-input': "") + ' p-2 relative'}> 
            <label ref={labelRef} className={props.labelStyle + (props.required ? ' create-course-input-label ': "") + (props.value !== '' && props.value != undefined ? ' top-[0.55rem] text-[0.86rem] ' : '') + ' relative h-4 whitespace-nowrap block w-fit pointer-events-none rounded-lg top-7 left-3 bg-main text-[#858FAB] px-1 transition-all duration-300'}>{props.placeholder}</label>
            <textarea value={props.value} onChange={(e) => {setCharacterLeft(250 - e.target.value.length); props.onChange ? props.onChange(e): null}} onFocus={moveLabel} onBlur={(e) => returnToInitial(e)} maxLength={250} className={props.style + ' h-20 sb-max:h-39 py-2 focus:outline-none resize-none w-full transition-bg bg-transparent rounded-lg pl-2 border-1.5 focus:border-[#0B80F3] border-[#A7AEC2]'}></textarea>
    ...
    return (
        <div ref={ref as any} className={props.inputDivStyle + ' w-auto p-2 min-w-[8.9rem]'}>
            <label ref={labelRef} className={(props.required ? 'create-course-input-label ': "") + props.labelStyle + (props.value !== '' && props.value != undefined ? ' top-[0.55rem] text-[0.86rem] ' : '') + ' relative whitespace-nowrap block w-fit pointer-events-none rounded-lg top-7 h-4 left-2 bg-main text-[#858FAB] px-1 transition-all duration-300'}>{props.placeholder}</label>
