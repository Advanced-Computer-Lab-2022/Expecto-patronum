...
const SearchBar = (props: Props) => {
  ...
  return (
    <form className='flex items-center w-full'>
        <input id='search-value' ref={searchInputRef} value={searchValue} onChange={(e) => setSearchValue(e.target.value)} placeholder='Search for anything' className='rounded-full transition-outline-width duration-200 nv-max:absolute relative max-w-lg nv-max:w-11/12 nv-max:mx-auto text-white nv-max:left-0 nv-max:right-0 w-96 h-8 pl-2.5 bg-navlink-bg placeholder:italic placeholder:text-sm placeholder-white tracking-wide placeholder-opacity-70 focus:outline focus:outline-2 outline-searchFocus'/>
        <button id='close-btn' type='button' onClick={closeSearch} className='absolute text-white rounded-full nv-max:bg-red-500 hidden scale-150 right-26 top-6 hover:scale-160 hover:cursor-pointer transition-transform duration-300' ><IoMdClose /></button>
        <button id='submit-search-btn' ref={searchRef} type={type} disabled={isDisabled} onMouseOver={disableCursor} onClick={(e) => toggleSearchInput(e)} className='rounded-full nv-max:absolute nv-max:right-24 nv-max:bg-white bg-navlink-bg p-2 relative right-8'>
