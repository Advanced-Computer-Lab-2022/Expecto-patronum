...
function Navbar() {
  ...
  return (
    ...
    >
      <div
        ref={parentRef}
        className="relative z-50 flex justify-between items-center nv:px-2 h-16 drop-shadow-lg shadow-lg bg-navbar"
      >
        <div className="nv-max:absolute z-behind flex bg-navbar">
          <img
            className="h-16 w-48 py-2 px-6"
            src="/images/Expecto Patronum (White).png"
          />
        </div>
        <div className="flex items-center">
          ...
          <div
            ref={curtainRef}
            className={
              "mob:w-screen nv-max-mob:w-fullscreen nv-max:relative z-10 transition-navbar-anime duration-1000 nv-max:bottom-36 nv-max:bg-navbar"
            }
            onMouseLeave={hide}
          >
            <a
              className="navbar-link text-navlink py-3 px-4 nv-max:mx-1 whitespace-nowrap z-10 relative nv-max:block transition-all duration-300"
              href=""
              onMouseOver={(e) => autoMove(e)}
            >
              Services
            </a>
            <a
            ...
            <div
              ref={hoverRef}
              id="movable-hover"
              className="absolute hidden right-0 z-last transition-all nv:z-behind nv-max:z-0 duration-200 bg-navlink-bg h-6 px-4 py-1 rounded-full"
            ></div>
          </div>
        </div>
      </div>
