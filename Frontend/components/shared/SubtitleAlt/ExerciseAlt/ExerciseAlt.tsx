...
const ExerciseAlt = (props: Props) => {
  ...
  function setQuestion(index: number, e: React.ChangeEvent<HTMLInputElement>) {
    const values = [...props.subtitles];
    values[props.subtitleIndex].exercise.questions[index].question =
      e.target.value;
    props.setSubtitles(values);
  }
  ...
  function setChoice(
    ...
    const values = [...props.subtitles];
    values[props.subtitleIndex].exercise.questions[questionIndex].choices[
      index
    ] = e.target.value;
    props.setSubtitles(values);
  }
  ...
  function removeChoice(questionIndex: number, childIndex: number) {
    ...
    values[props.subtitleIndex].exercise.questions[
      questionIndex
    ].choices.splice(childIndex, 1);
    props.setSubtitles(values);
  }
  ...
  function setCorrectAnswer(
    ...
    const values = [...props.subtitles];
    values[props.subtitleIndex].exercise.questions[questionIndex].answer =
      e.target.value;
    props.setSubtitles(values);
  }
  ...
  function toggleQuestion(
    ...
    e.target.children[1].classList.toggle("rotate-negative-90");
    const question = document.getElementById(
      "question-" + questionIndex + "-of-subtitle-" + props.subtitleIndex
    );
  ...
  function showHint(questionIndex: number) {
    ...
    <div className="px-2 my-1 border-2 border-gray-400 border-opacity-25 rounded-xl">
      {
        <div>
          ...
          <div>
            {props.subtitles[props.subtitleIndex].exercise.questions.map(
              (q: any, questionIndex: number) => (
                <div
                  ...
                  key={questionIndex}
                  className="relative transition-all overflow-hidden px-2 my-2 sb-max:mx-6 bg-white max-h-10"
                >
                  <button
                    type="button"
                    onClick={(e) =>
                      toggleQuestion(e, questionIndex)
                    }
                    className={
                      (questionIndex % 2 === 0
                        ? "bg-gray-300 "
                        : "bg-gray-200 ") +
                      " w-full h-8 relative z-20 text-left flex items-center rounded-md"
                    }
                  >
                    <span className="rounded-md bg-black text-white ml-2 px-2">
                      <span className="bg-black h-3 w-3 block absolute bottom-2.5 left-6.5 rotate-45"></span>
                      {questionIndex + 1}
                    </span>
                    <RiArrowDropDownLine className="pointer-events-none scale-200 transition-all duration-200 ml-3 rotate-negative-90" />
                  ...
                  <div className="relative transition-all duration-1000 px-3">
                    <div>
                      <Input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setQuestion(questionIndex, e)
                        }
                        inputDivStyle="col-12"
                      />
                      <button
                        type="button"
                        onClick={() => removeQuestion(questionIndex)}
                        className="text-red-600 hover:text-red-500 text-xs relative bottom-1 left-6 h-0 whitespace-nowrap"
                      >
                        Remove Question
                      </button>
                      <button
                        ...
                        className="hover:text-black text-navlink-bg text-xs relative bottom-1 left-12 h-0 whitespace-nowrap"
                      >
                        Add Choice
                      </button>
                    </div>
                    <div className="row pl-4">
                      {q.choices.map((c: any, childIndex: number) => (
                        <div className="col-md-6" key={childIndex}>
                          <Input
                            ...
                            onChange={(
                              e: React.ChangeEvent<HTMLInputElement>
                            ) => setChoice(questionIndex, childIndex, e)}
                          />
                          <button
                            type="button"
                            onClick={() =>
                              removeChoice(questionIndex, childIndex)
                            }
                            className="text-red-600 hover:text-red-500 text-xs relative bottom-2 left-6 h-0 whitespace-nowrap"
                          >
                            Delete
                          </button>
                        </div>
                      ...
                      <div className="col-md-6">
                        {/* <AiOutlineInfoCircle onMouseOver={() => showHint(questionIndex)} className="absolute text-navlink-bg right-7 bottom-14 mb-1" /> */}
                        ...
                        <Input
                          ...
                          value={q.answer}
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            setCorrectAnswer(e, questionIndex)
                          }
                          labelStyle="left-1.5"
                      ...
                      <p
                        id={"question-message-" + questionIndex + "-of-subtitle-" + props.subtitleIndex}
                        className="hidden text-center relative bottom-3 text-red-600"
                      ></p>
                    </div>
                  </div>
                </div>
          ...
          <button
            type="button"
            onClick={addQuestion}
            className="hover:text-white text-navlink-bg text-xs mx-auto flex transition-all duration-200 hover:bg-navlink-bg border-navlink-bg border-px rounded-md p-2 m-2 whitespace-nowrap"
          >
            Add Question
          </button>
        </div>
