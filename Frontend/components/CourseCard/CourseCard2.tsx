    ...
    <div className={`${!props.isViewList ? 'flex-col': 'sb-max:flex-col'} flex p-3 relative bg-white mb-8 rounded-xl shadow-md hover:shadow-lg transition-all duration-200`}>
        ...
        <div className={`${!props.isViewList ? 'mb-4': 'sb-max:mb-4 ml-3'} flex-col w-full min-w-0 justify-between h-fit text-[#384557]`}>
            ...
            <div className='min-w-0 row p-0 flex items-center justify-between text-sm'>
                ...
                <label className='whitespace-nowrap text-[#8d95a1] col'>Students: <span className='font-bold text-[#384557]'>{props.course.purchases.toLocaleString()}</span></label>
            </div>
        ...
        <label className='right-3 bottom-3 whitespace-nowrap absolute text-sm font-bold text-[#038470] bg-[#D7FFE0] px-2 py-0.5 rounded-full'>
            <span className={`${parseInt(props.course.discount?.discount) === 0 ? '': `${lineThrough} text-xs text-gray-500`} relative`}>{Rate.curr} {(Math.floor(props.course.price * Rate.rate) - 0.01).toLocaleString()}</span>
            <span className={`${parseInt(props.course.discount?.discount) === 0 ? 'hidden': ''} ml-2`}>{getDiscountPrice()}</span>
        </label>
        <label className={`${parseInt(props.course.discount?.discount) === 0 ? 'hidden': saleRibbon}`}><span className='absolute text-white top-[3.75rem] left-2 z-[1] whitespace-nowrap -rotate-45'>{props.course.discount?.discount === 100 ? 'FOR FREE' : `SAVE ${props.course.discount?.discount}%`}</span></label>
   </div>
  )
}
