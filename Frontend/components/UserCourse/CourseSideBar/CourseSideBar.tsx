...
const CourseSideBar = (props: Props) => {
  ...
  useEffect(() => {
    //@ts-ignore
    if (ContentChoosen.isExercise && ContentChoosen.data.id == "" || !ContentChoosen.isExercise && ContentChoosen.data.url == "") {
      let subtitleIndex = subtitlesData.findIndex((x) => x._id == ContentChoosen.SubtitleID);
      let contentIndex = subtitlesData[subtitleIndex].contents.findIndex((x) => x._id == ContentChoosen.ContentID);
      SetContentChoosen((prev) => {
        return {
  ...
  return (
    <div className={SideBarContainer + " " + (FixSideBar && FixedSideBar)}>
      <div className={SideBarHeaderContainer}>
        <h1 className={SideBarHeader}>Course Content</h1>
        <AiOutlineClose cursor={'pointer'} fontSize={20} />
      </div>
...
const SideBarContainer = classNames("w-[30%] shadow-md bg-main");
